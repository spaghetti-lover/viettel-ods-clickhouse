...
ORDER BY (created_at, transaction_type) TTL ttl + INTERVAL 7 DAY;
CREATE TABLE IF NOT EXISTS _kafka_transactions (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
SELECT JSONExtractString(message, 'after.order_id') AS order_id,
...
FROM _kafka_transactions SETTINGS stream_like_engine_allow_direct_select = 1;
...
ORDER BY (id, user_id) TTL ttl + INTERVAL 7 DAY;
CREATE TABLE IF NOT EXISTS _kafka_orders (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
SELECT JSONExtractString(message, 'after.id') AS id,
    ...
    JSONExtractString(message, 'after.created_at') AS created_at
FROM _kafka_orders SETTINGS stream_like_engine_allow_direct_select = 1;
-- Products table ---------------------------------------------
...
CREATE TABLE IF NOT EXISTS _kafka_products (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
SELECT toUInt32(JSONExtractString(message, 'after.id')) AS id,
    ...
    toFloat32(
        JSONExtractString(message, 'after.product_price')
    ) AS product_price
FROM _kafka_products SETTINGS stream_like_engine_allow_direct_select = 1;
-- Tags table ------------------------------------------
CREATE TABLE IF NOT EXISTS tags (id UInt32, tag_name String) ENGINE = MergeTree
ORDER BY (id);
CREATE TABLE IF NOT EXISTS _kafka_tags (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.tags',
kafka_group_name = 'clickhouse_tags_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tags to tags (id String, tag_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.tag_name') AS tag_name
FROM _kafka_tags SETTINGS stream_like_engine_allow_direct_select = 1;
-- Brands table ---------------------------------------------------
CREATE TABLE IF NOT EXISTS brands (id UInt32, brand_name String) ENGINE = MergeTree
ORDER BY (id);
CREATE TABLE IF NOT EXISTS _kafka_brands (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.brands',
kafka_group_name = 'clickhouse_brands_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_brands to brands (id String, brand_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.brand_name') AS brand_name
FROM _kafka_brands SETTINGS stream_like_engine_allow_direct_select = 1;
-- Users table -------------------------------------------------------
...
ORDER BY (id);
CREATE TABLE IF NOT EXISTS _kafka_users (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
SELECT JSONExtractString(message, 'after.id') AS id,
    ...
    JSONExtractString(message, 'after.created_at') AS created_at
FROM _kafka_users SETTINGS stream_like_engine_allow_direct_select = 1;
-- roles table ------------------------------------------------
CREATE TABLE IF NOT EXISTS roles (id UInt32, role_name String) ENGINE = MergeTree
ORDER BY (id, role_name);
CREATE TABLE IF NOT EXISTS _kafka_roles (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.roles',
kafka_group_name = 'clickhouse_roles_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_roles to roles (id String, role_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.role_name') AS role_name
FROM _kafka_roles SETTINGS stream_like_engine_allow_direct_select = 1;
-- role_user table ----------------------------------------------
...
CREATE TABLE IF NOT EXISTS _kafka_role_user (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
SELECT JSONExtractString(message, 'after.id') AS id,
    ...
    JSONExtractString(message, 'after.user_id') AS user_id
FROM _kafka_role_user SETTINGS stream_like_engine_allow_direct_select = 1;
-- provinces table --------------------------------------------
...
CREATE TABLE IF NOT EXISTS _kafka_provinces (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.provinces',
kafka_group_name = 'clickhouse_provinces_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_provinces to provinces (id String, province_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.province_name') AS province_name
FROM _kafka_provinces SETTINGS stream_like_engine_allow_direct_select = 1;
-- Cities table -------------------------------------------------
...
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_cities (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
FROM _kafka_cities SETTINGS stream_like_engine_allow_direct_select = 1;
-- Addresses table ---------------------------------------------
...
ORDER BY (id) SETTINGS index_granularity = 8192;
-- Optional, adjust based on your needs
CREATE TABLE IF NOT EXISTS _kafka_addresses (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
FROM _kafka_addresses SETTINGS stream_like_engine_allow_direct_select = 1;
-- Categories Table ---------------------------------------------
...
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_categories (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
FROM _kafka_categories SETTINGS stream_like_engine_allow_direct_select = 1;
-- Product_Tag Table -------------------------------------
CREATE TABLE IF NOT EXISTS product_tag (product_id UInt32, tag_id UInt32) ENGINE = MergeTree
ORDER BY (tag_id, product_id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_product_tag (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.product_tag',
kafka_group_name = 'clickhouse_product_tag_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_product_tag to product_tag (product_id String, tag_id String) AS
SELECT JSONExtractString(message, 'after.product_id') AS product_id,
    JSONExtractString(message, 'after.tag_id') AS tag_id
FROM _kafka_product_tag SETTINGS stream_like_engine_allow_direct_select = 1;
-- AdsCampaign Table ----------------------------------
...
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_adscampaign (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.adscampaigns',
kafka_group_name = 'clickhouse_product_adscampaigns_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_adscampaigns to adscampaigns (id String, campaign_title String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.campaign_title') AS campaign_title
FROM _kafka_adscampaign SETTINGS stream_like_engine_allow_direct_select = 1;
-- Discounts Table -------------------------------------------------
...
ORDER BY (id, started_at) SETTINGS index_granularity = 8192;
-- Optional, adjust based on your needs
CREATE TABLE IF NOT EXISTS _kafka_discounts (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
FROM _kafka_discounts SETTINGS stream_like_engine_allow_direct_select = 1;
-- OrderStatus Table -------------------------------------------
CREATE TABLE IF NOT EXISTS orderstatus (id UInt32, order_status_name String) ENGINE MergeTree
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_orderstatus (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.orderstatus',
kafka_group_name = 'clickhouse_orderstatus_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_orderstatus to orderstatus (id String, order_status_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.order_status_name') AS order_status_name
FROM _kafka_orderstatus SETTINGS stream_like_engine_allow_direct_select = 1;
-- PaymentMethods Table -------------------------------------------
CREATE TABLE IF NOT EXISTS paymentmethods (id UInt32, payment_method_name String) ENGINE MergeTree
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_paymentmethods (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.paymentmethods',
kafka_group_name = 'clickhouse_paymentmethods_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_paymentmethods to paymentmethods (id String, payment_method_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.payment_method_name') AS payment_method_name
FROM _kafka_paymentmethods SETTINGS stream_like_engine_allow_direct_select = 1;
-- Payment Status Table-------------------------------------
CREATE TABLE IF NOT EXISTS paymentstatus (id UInt32, payment_status_name String) ENGINE MergeTree
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_paymentstatus (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.paymentstatus',
kafka_group_name = 'clickhouse_paymentstatus_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_paymentstatus to paymentstatus (id String, payment_status_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.payment_status_name') AS payment_status_name
FROM _kafka_paymentstatus SETTINGS stream_like_engine_allow_direct_select = 1;
-- Shipping Status Table-------------------------------------
CREATE TABLE IF NOT EXISTS shippingstatus (id UInt32, shipping_status_name String) ENGINE MergeTree
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_shippingstatus (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.shippingstatus',
kafka_group_name = 'clickhouse_shippingstatus_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_shippingstatus to shippingstatus (id String, shipping_status_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.shipping_status_name') AS shipping_status_name
FROM _kafka_shippingstatus SETTINGS stream_like_engine_allow_direct_select = 1;
-- Shipping Methods Table-------------------------------------
CREATE TABLE IF NOT EXISTS shippingmethods (id UInt32, shipping_method_name String) ENGINE MergeTree
ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE IF NOT EXISTS _kafka_shippingmethods (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
kafka_topic_list = 'ecommerce_cdc.public.shippingmethods',
kafka_group_name = 'clickhouse_shippingmethods_consumer',
kafka_format = 'JSONAsString',
kafka_num_consumers = 1,
kafka_max_block_size = 1048576;
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_shippingmethods to shippingmethods (id String, shipping_method_name String) AS
SELECT JSONExtractString(message, 'after.id') AS id,
    JSONExtractString(message, 'after.shipping_method_name') AS shipping_method_name
FROM _kafka_shippingmethods SETTINGS stream_like_engine_allow_direct_select = 1;
-- OrderDetails Table-------------------------------------
...
ORDER BY (order_id, product_id) SETTINGS index_granularity = 8192;
-- Optional, adjust based on your needs
CREATE TABLE IF NOT EXISTS _kafka_orderdetails (`message` String) ENGINE Kafka SETTINGS kafka_broker_list = 'kafka-broker:19092',
...
FROM _kafka_orderdetails SETTINGS stream_like_engine_allow_direct_select = 1;
------------ Analytical Tables ------------
...
WHERE orders.order_status_id IN (
        SELECT id
        FROM orderstatus
...
ORDER BY (month);
select month,
    sumMerge(monthly_revenue),
    avgMerge(avg_order_value),
    countMerge(monthly_orders)
FROM _at_monthly_orders_status
group by month;
...
GROUP BY month SETTINGS stream_like_engine_allow_direct_select = 1;
...
CREATE TABLE IF NOT EXISTS _at_daily_user_registration (day UInt32, registration UInt32) ENGINE SummingMergeTree((registration)) PARTITION BY day
ORDER BY (day);
...
CREATE MATERIALIZED VIEW IF NOT EXISTS _mv_at_daily_user_registration TO _at_daily_user_registration (day UInt32, registration UInt32) AS
SELECT toYYYYMMDD(created_at) AS day,
...
-- Monthly registration counts are aggregated using the "registration" column.
-- The table is ordered by month for faster retrieval of sequential data.
CREATE TABLE IF NOT EXISTS _at_monthly_user_registration (month UInt32, registration UInt32) ENGINE SummingMergeTree((registration)) PARTITION BY month
ORDER BY (month);
...
-- The aggregated data is stored in the "_at_user_registration_statistics" table.
CREATE MATERIALIZED VIEW IF NOT EXISTS _mv_at_monthly_user_registration TO _at_monthly_user_registration (month UInt32, registration UInt32) AS
SELECT toYYYYMMDD(created_at) AS month,
    COUNT(id) AS registration
FROM users
GROUP BY month;
...
SELECT brand_name,
    countState(products.id) AS product_count
FROM products
...
SELECT province_name,
    countState(users.id) AS user_count
FROM users
...
CREATE MATERIALIZED VIEW IF NOT EXISTS _mv_at_total_number_of_products_sold_by_city TO _at_total_number_of_products_sold_by_city (city_name String, total_sold UInt32) AS
SELECT city_name,
...
select *
from _at_total_revenue_per_products
